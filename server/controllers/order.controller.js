...
/* ─────────────────────────────────────────────
   CREATE ORDER
───────────────────────────────────────────── */
const createOrder = asyncHandler(async (req, res, plantsCollection, ordersCollection, trackingCollection) => {
  const { plantId, quantity, customer, delivery, payment } = req.body;

  if (!plantId || !quantity || !customer?.phone || !delivery?.address) {
    return res.status(400).json({ success: false, message: "Missing required fields: phone and address are required" });
  }
  ...
  const plant = await plantsCollection.findOneAndUpdate(
    { _id: new ObjectId(plantId), status: { $ne: "flagged" }, quantity: { $gte: qty } },
    { $inc: { quantity: -qty }, $set: { updatedAt: new Date() } },
    { returnDocument: "before" },
  );

  if (!plant) {
    return res.status(409).json({ success: false, message: "Insufficient stock or plant unavailable" });
  }
...
/* ─────────────────────────────────────────────
   GET ORDERS
───────────────────────────────────────────── */
const getOrders = asyncHandler(async (req, res, ordersCollection, usersCollection) => {
  const {
  ...
  const [orders, totalCount] = await Promise.all([
    ordersCollection.find(query).sort({ createdAt: -1 }).skip(skip).limit(limitNum).toArray(),
...
const getSingleOrder = asyncHandler(async (req, res, ordersCollection) => {
  ...
  const email = req.user.email;
  if (req.userRole !== "admin" && order.customer.email !== email && order.seller.email !== email) {
...
───────────────────────────────────────────── */
const updateOrderStatus = asyncHandler(async (req, res, ordersCollection, plantsCollection, trackingCollection) => {
  const { id }     = req.params;
  ...
  await ordersCollection.updateOne({ _id: new ObjectId(id) }, { $set: setPayload });
...
───────────────────────────────────────────── */
const deleteOrder = asyncHandler(async (req, res, ordersCollection, plantsCollection, trackingCollection) => {
  const { id } = req.params;
  if (!ObjectId.isValid(id)) {
    return res.status(400).json({ success: false, message: "Invalid order ID" });
  }
  ...
  if (order.status === "delivered") {
    return res.status(409).json({
      success: false,
      message: "Delivered orders cannot be deleted — they are permanent records",
    });
  }
...
module.exports = { createOrder, getOrders, getSingleOrder, updateOrderStatus, deleteOrder };